...
export default function TaskList({ tasks, onComplete, onDelete }) {
  if (!tasks.length) {
    return <div className="card"><p className="muted">No tasks yet.</p></div>;
  }
        ...
        <div className="card" key={task.id}>
          <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p className="muted">Attachments: {task.attachment_count}</p>
            </div>
            <div className="row">
              <Link className="link" href={`/tasks/${task.id}`}>View</Link>
